...
(delete-selection-mode t)             ; typing on region deletes region
...
** Info: explicit package installation
...
The variable =package-selected-packages= stores all packages installed explicitly by the user.

You can install anything that's not in that list with:
#+begin_example
...
You can also set this variable with:
#+begin_example
(setq package-selected-packages
      '(multiple-cursors ivy-rich bind-key acme-theme minions))
#+end_example
...
(defun oht/org-mode-hook ()
  ...
  ;; this shouldn't be needed, debugging
  (undo-tree-mode)
  )
...
|----------+--------------+--------------------------|
| red      | Stay Alive   | Pass-thru and Exit       |
| pink     | Stay Alive   | Pass-thru and Stay Alive |
| amaranth | Stay Alive   | Reject and Stay Alive    |
| teal     | Exit         | Reject and Stay Alive    |
| blue     | Exit         | Pass-thru and Exit       |
...
;; since ctrl+alt+b/f are system shortcuts, respect that
...
#+begin_src emacs-lisp
(bind-key "s-' a" 'org-agenda)
(bind-key "s-' b" 'counsel-descbinds)
(bind-key "s-' c" 'org-capture)
(bind-key "s-' d" 'sdcv-search)
(bind-key "s-' f" 'oht/counsel-find-org)
(bind-key "s-' h" 'hl-line-mode)
(bind-key "s-' l" 'oht/toggle-line-numbers)
(bind-key "s-' o" 'counsel-outline)
(bind-key "s-' s-o" 'counsel-org-goto-all)
(bind-key "s-' w" 'oht/toggle-whitespace)
...
#+begin_src emacs-lisp
(bind-key "s-= s" 'oht/split-below)
(bind-key "s-= v" 'oht/split-beside)
(bind-key "s-= h" 'hydra-windows/body)
(bind-key "s-= k" 'delete-window)
(bind-key "s-= o" 'delete-other-windows)
(bind-key "s-= b" 'balance-windows)
#+end_src
