...
- Backups stored in the cloud are encrypted before they leave my computer
  with Arq.
- Use encrypted transfers/sync (such as SSH, TLS) such as Syncthing.
...
- Arq 5 (NOT v6)
    + When using a private/public key pair for authentication, don't forget
      that Arq expects a PEM-encoded one. Created with =ssh-keygen -m pem=.
- BBEdit
...
The Mac home folder is a kind of dumping ground. There are a bunch of folders
you can't delete, even if you don't use them, a few have very strict security
requirements (Documents, Desktop, Downloads; which is what prompted this), and
seemingly every command-line app [[https://0x46.net/thoughts/2019/02/01/dotfile-madness/][wants to install a dotfile there]] without so
much as asking. It very much feels like a place that's for your computer's
use, and you can join-in so long as you don't break the rules. Which is fine,
I get why it is that way, but there's something to be said for a true home
that you can do anything with.

So I pretty much organize everything under =~/home/=.

- =~/home/=
    + =dot= - My dotfiles.
    + =files= - Anything that doesn't fit elsewhere.
    + =src= - Source code, and source code related things.
    + =opt= - Basically a place for software installed via a =git clone=. I don't back this folder up.
    + =org= - My org-files.
    + =work=
    + =project_a=
    + =project_b=
