...
Due to historical reasons, emacs thinks =C-i= is the same as =TAB= and =C-m= is the same as =RETURN=. The below
...
(require 'sdcv-mode)            ;dictionary lookup, requires sdcv
...
You can ensure packages are installed with something like:
...
(defun oht/shell-command-on-region ()
  "Like shell-command-on-region but replaces region."
  (interactive)
  (if (use-region-p)
