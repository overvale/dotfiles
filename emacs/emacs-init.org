...
(set-default 'cursor-type 'box)            ; use bar here, box in modes
...
(defun oht/set-font ()
  (interactive)
  "This function sets Iosevka Fixed SS08 to the default and fixed-pitched faces, and IBM Plex Serif to the variable-pitch face."
...
This package takes vim's concept of an 'object' (character, word, line, paragraph, etc.) and allows you to navigate and edit those objects. It also takes [[http://kakoune.org][Kakoune's]] concept of showing you those objects before you act on them (objed seems to make use of Emacs's Secondary Selection for this).  It also takes over movement commands (like =C-n=) to activate the selection of these objects. In some ways this is trying to solve the same problem [[https://github.com/emacsorphanage/god-mode][god-mode]] is targeting, or a more Emacs-like =evil-mode=.

Since this package radically redefines your interaction with Emacs, I've not enabled =objecd-mode= by default.
...
The object navigation/editing bindings are well thought-out, but I had to copy them here to understand them.

| Key   | Action                                                         |
|-------+----------------------------------------------------------------|
| M-SPC | Activate with default object                                   |
| M-#   | Prompt for object to activate with                             |
| h/l   | forward/back character                                         |
| H/L   | shift char back/forward                                        |
| s/r   | next/previous word                                             |
| S/R   | shift word back/forward                                        |
| n/p   | next/previous line                                             |
| N/P   | shift line forward/back                                        |
| v     | line object, move to start                                     |
| a     | line object, move to start                                     |
| e     | line object, move to end                                       |
| j     | move to other side of object                                   |
| t     | toggle between inner and whole object                          |
| []    | previous/next of same object                                   |
| i     | delete object and insert                                       |
| k     | kill object, select next                                       |
| K     | kill object, select previous                                   |
| d     | delete object, select next                                     |
| D     | delete object, select previous                                 |
| w     | copy object                                                    |
| y     | yank, cycle                                                    |
| ;     | comment object                                                 |
| !     | run object as shell command                                    |
| &     | pipe region through shell command                              |
| ~     | undo in object region                                          |
| "     | add "surrounding"                                              |
| m     | add/remove current object to "marked objects", select next     |
| M     | add/remove current object to "marked objects", select previous |
| /     | undo last edit                                                 |
| g     | exit                                                           |

* Scrolling

Scrolling in Emacs using the default keybindings (not the track-pad or mouse-wheel) is fast but sudden and sometimes disorienting. This disorientation is probably the reason I depend on my track-pad perhaps more than I should. [[https://with-emacs.com/posts/ui-hacks/keep-scrollin-scrollin-scrollin/][(with-emacs]] has an interesting solution to this problem. It works by highlighting either the top or bottom line, performing the scroll, leaving the line highlighted, and then deactivating the highlight. The result is you can see the highlighted line move in the buffer, which gives you just a little bit of context.
